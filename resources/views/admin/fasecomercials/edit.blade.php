...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.fasecomercials.update", [$fasecomercial->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="comentarios">{{ trans('cruds.fasecomercial.fields.comentarios') }}</label>
                <input class="form-control {{ $errors->has('comentarios') ? 'is-invalid' : '' }}" type="text" name="comentarios" id="comentarios" value="{{ old('comentarios', $fasecomercial->comentarios) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('estado') ? 'is-invalid' : '' }}" type="text" name="estado" id="estado" value="{{ old('estado', $fasecomercial->estado) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('id_proyecto') ? 'is-invalid' : '' }}" name="id_proyecto_id" id="id_proyecto_id">
                    @foreach($id_proyectos as $id => $entry)
                        <option value="{{ $id }}" {{ (old('id_proyecto_id') ? old('id_proyecto_id') : $fasecomercial->id_proyecto->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.fasecomercial.fields.id_proyecto_helper') }}</span>
            </div>
            <div class="form-group">
