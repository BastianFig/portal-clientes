...
class FasecontableController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            $query = Fasecontable::with(['id_proyecto'])->select(sprintf('%s.*', (new Fasecontable)->table));
            ...
            $table->editColumn('actions', function ($row) {
                $viewGate      = 'fasecontable_show';
                $editGate      = 'fasecontable_edit';
                $deleteGate    = 'fasecontable_delete';
                $crudRoutePart = 'fasecontables';
            ...
            $table->editColumn('anticipo_50', function ($row) {
                return $row->anticipo_50 ? '<a href="' . $row->anticipo_50->getUrl() . '" target="_blank">' . trans('global.downloadFile') . '</a>' : '';
            });
            $table->editColumn('comentario', function ($row) {
                return $row->comentario ? $row->comentario : '';
            });
            $table->editColumn('anticipo_40', function ($row) {
                return $row->anticipo_40 ? '<a href="' . $row->anticipo_40->getUrl() . '" target="_blank">' . trans('global.downloadFile') . '</a>' : '';
            });
            $table->editColumn('anticipo_10', function ($row) {
                return $row->anticipo_10 ? '<a href="' . $row->anticipo_10->getUrl() . '" target="_blank">' . trans('global.downloadFile') . '</a>' : '';
            });
            ...
            $table->rawColumns(['actions', 'placeholder', 'anticipo_50', 'anticipo_40', 'anticipo_10', 'id_proyecto']);

            return $table->make(true);
        }

        return view('admin.fasecontables.index');
    }
    ...
    public function create()
    {
        ...
        return view('admin.fasecontables.create', compact('id_proyectos'));
    }
    ...
    public function store(StoreFasecontableRequest $request)
    {
        ...
        return redirect()->route('admin.fasecontables.index');
    }
    ...
    public function edit(Fasecontable $fasecontable)
    {
        ...
        return view('admin.fasecontables.edit', compact('fasecontable', 'id_proyectos'));
    }
    ...
    public function show(Fasecontable $fasecontable)
    {
        ...
        return view('admin.fasecontables.show', compact('fasecontable'));
    }
