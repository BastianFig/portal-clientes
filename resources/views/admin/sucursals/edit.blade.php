    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.sucursals.update", [$sucursal->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('empresa') ? 'is-invalid' : '' }}" name="empresa_id" id="empresa_id" required>
                    @foreach($empresas as $id => $entry)
                        <option value="{{ $id }}" {{ (old('empresa_id') ? old('empresa_id') : $sucursal->empresa->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nombre') ? 'is-invalid' : '' }}" type="text" name="nombre" id="nombre" value="{{ old('nombre', $sucursal->nombre) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('region') ? 'is-invalid' : '' }}" name="region" id="region">
                    <option value="" disabled {{ old('region', $sucursal->region) === null ? 'selected' : '' }}>
                        {{ trans('global.pleaseSelect') }}
                    </option>
                    @foreach(App\Models\Sucursal::REGION_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('region', $sucursal->region) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('comuna') ? 'is-invalid' : '' }}" name="comuna" id="comuna">
                    ...
                    <option value="{{  $sucursal->comuna }}" selected>{{  $sucursal->comuna }}</option>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('direccion_sucursal') ? 'is-invalid' : '' }}" type="text" name="direccion_sucursal" id="direccion_sucursal" value="{{ old('direccion_sucursal', $sucursal->direccion_sucursal) }}">
