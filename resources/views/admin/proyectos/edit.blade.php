    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.proyectos.update", [$proyecto->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('id_cliente') ? 'is-invalid' : '' }}" name="id_cliente_id" id="id_cliente_id" required>
                    @foreach($id_clientes as $id => $entry)
                        <option value="{{ $id }}" {{ (old('id_cliente_id') ? old('id_cliente_id') : $proyecto->id_cliente->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('sucursal') ? 'is-invalid' : '' }}" name="sucursal_id" id="sucursal_id">
                    @if($proyecto->sucursal)
                        <option value="{{ $proyecto->sucursal->id }}" selected>{{ $proyecto->sucursal->nombre }}</option>
                    @endif
                </select>
            ...
            <div class="form-group">
                <label class="required" for="id_usuarios_clientes">{{ trans('cruds.proyecto.fields.id_usuarios_cliente') }}</label>
                <div style="padding-bottom: 4px">
                    <span class="btn btn-info btn-xs select-all" style="border-radius: 0">{{ trans('global.select_all') }}</span>
                    <span class="btn btn-info btn-xs deselect-all" style="border-radius: 0">{{ trans('global.deselect_all') }}</span>
                </div>
                <select class="form-control select2 {{ $errors->has('id_usuarios_clientes') ? 'is-invalid' : '' }}" name="id_usuarios_clientes[]" id="id_usuarios_clientes" multiple required>
                    @foreach($id_usuarios_clientes as $id => $id_usuarios_cliente)
                        <option value="{{ $id }}" {{ (in_array($id, old('id_usuarios_clientes', [])) || $proyecto->id_usuarios_clientes->contains($id)) ? 'selected' : '' }}>{{ $id_usuarios_cliente }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('categoria_proyecto') ? 'is-invalid' : '' }}" name="categoria_proyecto" id="categoria_proyecto">
                    <option value disabled {{ old('categoria_proyecto', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('estado') ? 'is-invalid' : '' }}" name="estado" id="estado">
                    ...
                    @foreach(App\Models\Proyecto::ESTADO_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('estado', $proyecto->estado) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('fase') ? 'is-invalid' : '' }}" name="fase" id="fase">
                    ...
                    @foreach(App\Models\Proyecto::FASE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('fase', $proyecto->fase) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('fase'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                <label class="required" for="nombre_proyecto">{{ trans('cruds.proyecto.fields.nombre_proyecto') }}</label>
                <input class="form-control {{ $errors->has('nombre_proyecto') ? 'is-invalid' : '' }}" type="text" name="nombre_proyecto" id="nombre_proyecto" value="{{ old('nombre_proyecto', $proyecto->nombre_proyecto) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('disenador') ? 'is-invalid' : '' }}" name="disenador" id="disenador">
                    <option value disabled {{ old('disenador', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
            ...
            <!--<div class="form-group">
                ...
                <select class="form-control {{ $errors->has('instalador') ? 'is-invalid' : '' }}" name="instalador" id="instalador">
                    ...
                    @foreach(App\Models\Proyecto::INSTALADOR_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('instalador', $proyecto->instalador) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
...
$('#sucursal_id').change(function( ){
    ...
    $.ajax({
        ...
        success: function(response){
             //console.log(response);
            $("#id_usuarios_clientes option").remove();
            $("#id_usuarios_clientes").append("<option value='0'>Seleccione Usuario</option>"); 
            for(let i = 0; i < response.length; i++){
                //console.log(response[i].text);
