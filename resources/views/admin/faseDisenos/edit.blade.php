...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.faseDiseno.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.fase-disenos.update", [$faseDiseno->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="descripcion">{{ trans('cruds.faseDiseno.fields.descripcion') }}</label>
                <input class="form-control {{ $errors->has('descripcion') ? 'is-invalid' : '' }}" type="text" name="descripcion" id="descripcion" value="{{ old('descripcion', $faseDiseno->descripcion) }}">
            ...
            <div class="form-group">
                <label for="imagenes">{{ trans('cruds.faseDiseno.fields.imagenes') }}</label>
            ...
            <div class="form-group">
                <label for="propuesta">{{ trans('cruds.faseDiseno.fields.propuesta') }}</label>
            ...
            <div class="form-group">
                <label for="estado">{{ trans('cruds.faseDiseno.fields.estado') }}</label>
                <input class="form-control {{ $errors->has('estado') ? 'is-invalid' : '' }}" type="text" name="estado" id="estado" value="{{ old('estado', $faseDiseno->estado) }}">
                @if($errors->has('estado'))
                    <div class="invalid-feedback">
                        {{ $errors->first('estado') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.faseDiseno.fields.estado_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="id_proyecto_id">{{ trans('cruds.faseDiseno.fields.id_proyecto') }}</label>
                <select class="form-control select2 {{ $errors->has('id_proyecto') ? 'is-invalid' : '' }}" name="id_proyecto_id" id="id_proyecto_id">
                    @foreach($id_proyectos as $id => $entry)
                        <option value="{{ $id }}" {{ (old('id_proyecto_id') ? old('id_proyecto_id') : $faseDiseno->id_proyecto->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.faseDiseno.fields.id_proyecto_helper') }}</span>
            </div>
            <div class="form-group">
...
Dropzone.options.imagenesDropzone = {
    ...
    init: function () {
@if(isset($faseDiseno) && $faseDiseno->imagenes)
          var files =
            {!! json_encode($faseDiseno->imagenes) !!}
              for (var i in files) {
              var file = files[i]
...
Dropzone.options.propuestaDropzone = {
    ...
    init: function () {
@if(isset($faseDiseno) && $faseDiseno->propuesta)
          var files =
            {!! json_encode($faseDiseno->propuesta) !!}
              for (var i in files) {
              var file = files[i]
              this.options.addedfile.call(this, file)
              file.previewElement.classList.add('dz-complete')
