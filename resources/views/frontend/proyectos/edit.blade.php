        ...
        <div class="row justify-content-center">
            <div class="col-md-12">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <form method="POST" action="{{ route('frontend.proyectos.update', [$proyecto->id]) }}"
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control select2" name="id_cliente_id" id="id_cliente_id" required>
                                    @foreach ($id_clientes as $id => $entry)
                                        <option value="{{ $id }}"
                                            {{ (old('id_cliente_id') ? old('id_cliente_id') : $proyecto->id_cliente->id ?? '') == $id ? 'selected' : '' }}>
                                            {{ $entry }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <div style="padding-bottom: 4px">
                                    <span class="btn btn-info btn-xs select-all"
                                        style="border-radius: 10px">{{ trans('global.select_all') }}</span>
                                    <span class="btn btn-info btn-xs deselect-all"
                                        style="border-radius: 10px">{{ trans('global.deselect_all') }}</span>
                                </div>
                                <select class="form-control select2" name="id_usuarios_clientes[]" id="id_usuarios_clientes"
                                    ...
                                    @foreach ($id_usuarios_clientes as $id => $id_usuarios_cliente)
                                        <option value="{{ $id }}"
                                            {{ in_array($id, old('id_usuarios_clientes', [])) || $proyecto->id_usuarios_clientes->contains($id) ? 'selected' : '' }}>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control select2" name="sucursal_id" id="sucursal_id">
                                    @foreach ($sucursals as $id => $entry)
                                        <option value="{{ $id }}"
                                            {{ (old('sucursal_id') ? old('sucursal_id') : $proyecto->sucursal->id ?? '') == $id ? 'selected' : '' }}>
                                            {{ $entry }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" name="tipo_proyecto" id="tipo_proyecto">
                                    ...
                                    @foreach (App\Models\Proyecto::TIPO_PROYECTO_SELECT as $key => $label)
                                        <option value="{{ $key }}"
                                            {{ old('tipo_proyecto', $proyecto->tipo_proyecto) === (string) $key ? 'selected' : '' }}>
                                            {{ $label }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" name="estado" id="estado">
                                    ...
                                    @foreach (App\Models\Proyecto::ESTADO_SELECT as $key => $label)
                                        <option value="{{ $key }}"
                                            {{ old('estado', $proyecto->estado) === (string) $key ? 'selected' : '' }}>
                                            {{ $label }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" name="fase" id="fase">
                                    ...
                                    @foreach (App\Models\Proyecto::FASE_SELECT as $key => $label)
                                        <option value="{{ $key }}"
                                            {{ old('fase', $proyecto->fase) === (string) $key ? 'selected' : '' }}>
                                            {{ $label }}</option>
                                    @endforeach
                                </select>
                                @if ($errors->has('fase'))
