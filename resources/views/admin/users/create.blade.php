    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.users.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                @if($rol_activo == 'Ventas')
                    <select class="form-control select2 {{ $errors->has('roles') ? 'is-invalid' : '' }}" name="roles[]" id="roles" multiple required>
                        <option value="2" {{ in_array(3, old('roles', [])) ? 'selected' : '' }}>User</option>
                    </select>
                @else
                    <select class="form-control select2 {{ $errors->has('roles') ? 'is-invalid' : '' }}" name="roles[]" id="roles" multiple required>
...
var id_empresa = $("#empresa_id").val();
 ...
 $.ajax({
    ...
    // Seleccionar los campos "name" y "telefono" usando jQuery
    const $nameInput = $('#name');
    const $telefonoInput = $('#telefono');
    const $passwordInput = $('#password');

    // Agregar evento de cambio para los campos "name" y "telefono"
    $nameInput.on('input', updatePassword);
    $telefonoInput.on('input', updatePassword);

    // Función para generar el valor predeterminado para el campo "password"
    function updatePassword() {
        const nameValue = $nameInput.val().substring(0, 4);
        const telefonoValue = $telefonoInput.val().slice(-4);
        const defaultValue = nameValue + telefonoValue;
