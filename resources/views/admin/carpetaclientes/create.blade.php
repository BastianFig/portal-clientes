...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.carpetacliente.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.carpetaclientes.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('id_fase_comercial') ? 'is-invalid' : '' }}" name="id_fase_comercial_id" id="id_fase_comercial_id">
                    @foreach($id_fase_comercials as $id => $entry)
                        <option value="{{ $id }}" {{ old('id_fase_comercial_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
