...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.fase-postventa.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('encuesta') ? 'is-invalid' : '' }}" name="encuesta_id" id="encuesta_id">
                    @foreach($encuestas as $id => $entry)
                        <option value="{{ $id }}" {{ old('encuesta_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('id_usuarios') ? 'is-invalid' : '' }}" name="id_usuarios[]" id="id_usuarios" multiple>
                    @foreach($id_usuarios as $id => $id_usuario)
                        <option value="{{ $id }}" {{ in_array($id, old('id_usuarios', [])) ? 'selected' : '' }}>{{ $id_usuario }}</option>
                    @endforeach
                </select>
