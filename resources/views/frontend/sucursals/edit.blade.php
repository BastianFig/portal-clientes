        ...
        <div class="row justify-content-center">
            <div class="col-md-12">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <form method="POST" action="{{ route('frontend.sucursals.update', [$sucursal->id]) }}"
                            ...
                            <div class="form-group">
                                ...
                                <input class="form-control input-custom"
                                    ...
                                    placeholder="Nombre" type="text" name="nombre" id="nombre"
                            ...
                            <div class="form-group">
                                <label
                                    for="direccion_sucursal">{{ trans('cruds.sucursal.fields.direccion_sucursal') }}</label>
                                <input class="form-control input-custom"
                                    style="border: none; background-color: #f8f8f8; border-radius: 10px;"
                                    placeholder="Dirección de la sucursal" type="text" name="direccion_sucursal"
                                ...
                                @endif
                                <span
                                    class="help-block">{{ trans('cruds.sucursal.fields.direccion_sucursal_helper') }}</span>
                            </div>
                            <div class="form-group">
                                ...
                                <select class="form-control" name="comuna" id="comuna">
                                    ...
                                    @foreach (App\Models\Sucursal::COMUNA_SELECT as $key => $label)
                                        <option value="{{ $key }}"
                                            {{ old('comuna', $sucursal->comuna) === (string) $key ? 'selected' : '' }}>
                                            {{ $label }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" name="region" id="region">
                                    ...
                                    @foreach (App\Models\Sucursal::REGION_SELECT as $key => $label)
                                        <option value="{{ $key }}"
                                            {{ old('region', $sucursal->region) === (string) $key ? 'selected' : '' }}>
                                            {{ $label }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control select2" name="empresa_id" id="empresa_id" required>
                                    @foreach ($empresas as $id => $entry)
                                        <option value="{{ $id }}"
                                            {{ (old('empresa_id') ? old('empresa_id') : $sucursal->empresa->id ?? '') == $id ? 'selected' : '' }}>
                                            {{ $entry }}</option>
                                    @endforeach
                                </select>
