            ...
            <div class="col-md-12">
                ...
                <div class="card">
                    <div class="card-header">
                        {{ trans('global.edit') }} {{ trans('cruds.fasecomercialproyecto.title_singular') }}
                    </div>

                    <div class="card-body">
                        <form method="POST"
                            action="{{ route('frontend.fasecomercialproyectos.update', [$fasecomercialproyecto->id]) }}"
                            enctype="multipart/form-data">
                            @method('PUT')
                            @csrf
                            <div
                                class="form-group input-custom p-4"style="border: none; background-color: #f8f8f8; border-radius: 10px;">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control select2" name="id_proyecto_id" id="id_proyecto_id">
                                    @foreach ($id_proyectos as $id => $entry)
                                        <option value="{{ $id }}"
                                            {{ (old('id_proyecto_id') ? old('id_proyecto_id') : $fasecomercialproyecto->id_proyecto->id ?? '') == $id ? 'selected' : '' }}>
                                            {{ $entry }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <input
                                    class="form-control input-custom"style="border: none; background-color: #f8f8f8; border-radius: 10px;"
                                    type="text" name="estado" id="estado"
                                    value="{{ old('estado', $fasecomercialproyecto->estado) }}" placeholder="Estado">
