...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.carpetaclientes.update", [$carpetacliente->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            ...
            <div class="form-group">
                <label for="id_fase_comercial_id">{{ trans('cruds.carpetacliente.fields.id_fase_comercial') }}</label>
                <select class="form-control select2 {{ $errors->has('id_fase_comercial') ? 'is-invalid' : '' }}" name="id_fase_comercial_id" id="id_fase_comercial_id">
                    @foreach($id_fase_comercials as $id => $entry)
                        <option value="{{ $id }}" {{ (old('id_fase_comercial_id') ? old('id_fase_comercial_id') : $carpetacliente->id_fase_comercial->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.carpetacliente.fields.id_fase_comercial_helper') }}</span>
            </div>
            <div class="form-group">
