...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.fasedespachos.update", [$fasedespacho->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('fecha_despacho') ? 'is-invalid' : '' }}" type="text" name="fecha_despacho" id="fecha_despacho" value="{{ old('fecha_despacho', $fasedespacho->fecha_despacho) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('estado_instalacion') ? 'is-invalid' : '' }}" name="estado_instalacion" id="estado_instalacion">
                    ...
                    @foreach(App\Models\Fasedespacho::ESTADO_INSTALACION_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('estado_instalacion', $fasedespacho->estado_instalacion) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('comentario') ? 'is-invalid' : '' }}" name="comentario" id="comentario">{!! old('comentario', $fasedespacho->comentario) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('recibe_conforme') ? 'is-invalid' : '' }}" name="recibe_conforme" id="recibe_conforme">
                    ...
                    @foreach(App\Models\Fasedespacho::RECIBE_CONFORME_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('recibe_conforme', $fasedespacho->recibe_conforme) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
           ...
           <!-- <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('id_proyecto') ? 'is-invalid' : '' }}" name="id_proyecto_id" id="id_proyecto_id">
                    @foreach($id_proyectos as $id => $entry)
                        <option value="{{ $id }}" {{ (old('id_proyecto_id') ? old('id_proyecto_id') : $fasedespacho->id_proyecto->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('estado') ? 'is-invalid' : '' }}" type="text" name="estado" id="estado" value="{{ old('estado', $fasedespacho->estado) }}">
