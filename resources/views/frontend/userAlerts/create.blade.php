                        ...
                        <form method="POST" action="{{ route('frontend.user-alerts.store') }}" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                ...
                                <input class="form-control input-custom"
                                    ...
                                    placeholder="Texto de la alerta" type="text" name="alert_text" id="alert_text"
                                    value="{{ old('alert_text', '') }}" required>
                            ...
                            <div class="form-group">
                                ...
                                <input class="form-control input-custom"
                                    style="border: none; background-color: #f8f8f8; border-radius: 10px;"
                                    placeholder="Link de la alerta" type="text" name="alert_link" id="alert_link"
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control select2" name="users[]" id="users" multiple>
                                    @foreach ($users as $id => $user)
                                        <option value="{{ $id }}"
                                            {{ in_array($id, old('users', [])) ? 'selected' : '' }}>{{ $user }}
                                        </option>
                                    @endforeach
                                </select>
                                @if ($errors->has('users'))
