            ...
            <div class="col-md-12">
                ...
                <div class="card">
                    <div class="card-header">
                        {{ trans('global.edit') }} {{ trans('cruds.fasedespacho.title_singular') }}
                    </div>

                    <div class="card-body">
                        <form method="POST" action="{{ route('frontend.fasedespachos.update', [$fasedespacho->id]) }}"
                            ...
                            <div class="form-group input-custom p-4"
                                ...
                                <div class="needsclick dropzone" id="guia_despacho-dropzone">
                                </div>
                            ...
                            <div class="form-group">
                                ...
                                <input class="form-control date input-custom"
                                    ...
                                    name="fecha_despacho" id="fecha_despacho"
                                    value="{{ old('fecha_despacho', $fasedespacho->fecha_despacho) }}">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" name="estado_instalacion" id="estado_instalacion">
                                    <option value disabled
                                        {{ old('estado_instalacion', null) === null ? 'selected' : '' }}>
                                        {{ trans('global.pleaseSelect') }}</option>
                                    @foreach (App\Models\Fasedespacho::ESTADO_INSTALACION_SELECT as $key => $label)
                                        <option value="{{ $key }}"
                                            {{ old('estado_instalacion', $fasedespacho->estado_instalacion) === (string) $key ? 'selected' : '' }}>
                                            {{ $label }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <textarea class="form-control ckeditor input-custom"
                                    style="border: none; background-color: #f8f8f8; border-radius: 10px;" name="comentario" id="comentario">{!! old('comentario', $fasedespacho->comentario) !!}</textarea>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" name="recibe_conforme" id="recibe_conforme">
                                    ...
                                    @foreach (App\Models\Fasedespacho::RECIBE_CONFORME_SELECT as $key => $label)
                                        <option value="{{ $key }}"
                                            {{ old('recibe_conforme', $fasedespacho->recibe_conforme) === (string) $key ? 'selected' : '' }}>
                                            {{ $label }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group input-custom p-4"
                                style="border: none; background-color: #f8f8f8; border-radius: 10px;">
                                <label
                                    for="galeria_estado_muebles">{{ trans('cruds.fasedespacho.fields.galeria_estado_muebles') }}</label>
                                <div class="needsclick dropzone" id="galeria_estado_muebles-dropzone">
                                </div>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control select2" name="id_proyecto_id" id="id_proyecto_id">
                                    @foreach ($id_proyectos as $id => $entry)
                                        <option value="{{ $id }}"
                                            {{ (old('id_proyecto_id') ? old('id_proyecto_id') : $fasedespacho->id_proyecto->id ?? '') == $id ? 'selected' : '' }}>
                                            {{ $entry }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <input class="form-control input-custom"
                                    style="border: none; background-color: #f8f8f8; border-radius: 10px;" type="text"
                                    name="estado" id="estado" value="{{ old('estado', $fasedespacho->estado) }}">
...
@section('scripts')
    <script>
        Dropzone.options.guiaDespachoDropzone = {
            url: '{{ route('frontend.fasedespachos.storeMedia') }}',
            maxFilesize: 4, // MB
            maxFiles: 1,
            addRemoveLinks: true,
            headers: {
                            ...
                            return loader.file
                                .then(function(file) {
                                    return new Promise(function(resolve, reject) {
                                        ...
                                        xhr.open('POST',
                                            '{{ route('frontend.fasedespachos.storeCKEditorImages') }}',
                                            true);
            ...
            init: function() {
                @if (isset($fasedespacho) && $fasedespacho->galeria_estado_muebles)
                    ...
                    for (var i in files) {
                        ...
                        file.previewElement.classList.add('dz-complete')
                        $('form').append('<input type="hidden" name="galeria_estado_muebles[]" value="' + file
                            .file_name + '">')
                    }
                @endif
            },
