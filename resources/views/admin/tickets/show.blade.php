                    ...
                    <div class="form-group">
                        <div class="d-flex">
                            <div class="form-group d-flex">
                                <a class="btn btn-primary mr-5" href="{{ route('admin.tickets.index') }}" style="border-radius:10px">Volver</a>
                                <form method="POST" action="{{route('admin.tickets.cerrarTicket')}}" enctype="multipart/form-data">
                                    @csrf
                                    <input type="hidden" id="id_vendedor" class="id_vendedor" name="id_vendedor" value="{{$ticket->proyecto->id_vendedor}}">
                                    <a class="btn btn-warning ml-5 mr-5" href="{{ route('admin.tickets.index') }}" style="border-radius:10px; color:black;">Asignar a vendedor</a>
                                </form>
                                <form method="POST" action="{{route('admin.tickets.cerrarTicket')}}" enctype="multipart/form-data">
                                    @csrf
                                    <input type="hidden" id="ticket_id" class="ticket_id" name="ticket_id" value="{{$ticket->id}}">
                                    @if($ticket->estado == "Activo")
                                        <input class="btn btn-danger text-white  ml-5" id="btn_finalizar_ticket" type="submit" value="Finalizar Ticket">
                                    @endif
                                </form>
                            </div>
                        </div>
                            ...
                            @if($ticket->estado == "Activo")
                                <form action="{{ route('admin.tickets.storeMensaje') }}" method="post">
                                    ...
                                    <div class="input-group mb-3 col-12">
                                        ...
                                        <textarea style="border-radius:10px;" class="form-control" name="mensaje" placeholder="Envía un nuevo mensaje..."
                                            aria-label="Recipient's new message" aria-describedby="button-addon2"></textarea>
